...
class Edit extends Component {
    ...
    renderContent(){
        ...
        const { excerptLimit, metaStyle, metaShow, catShow, showImage, metaSeparator, titleShow, catStyle, catPosition, titlePosition, excerptShow , imgAnimation, imgOverlayType, imgOverlay, metaList, metaListSmall, showSmallCat, readMore, readMoreText, readMoreIcon, overlayContentPosition, showSmallBtn, showSmallExcerpt, metaPosition, showFullExcerpt, layout, columnFlip, titleAnimation, customCatColor, onlyCatColor } = this.props.attributes;
        return(
    ...
    render() {
        ...
        if (blockId) { CssGenerator(this.props.attributes, 'post-grid-4', blockId); }
        return (
            <Fragment>
                <InspectorControls>
                    <Sections>
                        <Section slug="design" title={__('Design')}>
                            <Layout 
                                ...
                                data={[{ type:'layout', block:'post-grid-4', key:'layout', pro: true, options: [
                                    { icon: icons.pg4layout1, label: __('Layout 1'), value: 'layout1', pro: false },
                                    { icon: icons.pg4layout2, label: __('Layout 2'), value: 'layout2', pro: true },
                                    { icon: icons.pg4layout3, label: __('Layout 3'), value: 'layout3', pro: true },
                                    { icon: icons.pg4layout4, label: __('Layout 4'), value: 'layout4', pro: true },
                                ]}]}/>
                        ...
                        <Section slug="setting" title={__('Setting')}>
                            <GeneralContent store={store} 
                                ...
                                include={[
                                    { position: 1, data:{ type:'range',key:'overlayHeight', min:0, max:1000, step:1, unit:true, responsive:true, label:'Height' } },
                                    { data:{ type:'toggle',key:'columnFlip', label:'Flip' } }, 
                            ...
                            { paginationShow && <PaginationContent store={store} include={[{ position: 1, data:{ type:'select',key:'paginationType', label:'Pagination Type', options:[{value:'none',label:'None'},{value:'navigation',label:'Navigation'}] } }]} exclude={['paginationType','paginationAjax','loadMoreText']}/> }
